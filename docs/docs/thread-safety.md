...
| `RetryBudget` | Thread-safe shared feedback balance. Capacity and refund settings are immutable; token updates are atomic across independent shields and partitions. |
...
| `IKevlarRegistry`, `IShieldProvider` | Thread-safe singleton services. Reloading registry lookups return stable live-forwarding shields; keyed providers expose immutable snapshots through `Current`. |
