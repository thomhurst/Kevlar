...
- **Handling clauses are ambient within one fluent chain.** A clause applies to every reactive strategy after it until replaced. `Wrap` and `Compose` seal clauses at the composition boundary; strategies already inside keep their handling, while strategies appended afterwards use Kevlar's default unless you declare a new clause locally.
