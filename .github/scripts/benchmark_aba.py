...
def main():
    ...
    subprocess.run(["git", "merge-base", "--is-ancestor", commits["A"], commits["B"]],
                   check=True)
    settings = ["--warmupCount", "25", "--iterationTime", "1000",
                "--iterationCount", "15", "--outliers", "DontRemove"]
    metadata = {
        "commits": commits,
        "filters": filters,
    ...
    for phase, label in (("A1", "A"), ("B", "B"), ("A2", "A")):
        run(["dotnet", "build-server", "shutdown"], repository,
            evidence / f"{phase}-build-server-shutdown.log")
        ...
        run([*command, *settings, "--artifacts", str(evidence / phase)],
        ...
        for name, row in measured[phase].items():
            warmup = [sample for sample in row["Measurements"]
                      if sample["IterationMode"] == "Workload"
                      and sample["IterationStage"] == "Warmup"]
            seconds = sum(sample["Nanoseconds"] for sample in warmup) / 1e9
            if seconds < 20:
                raise RuntimeError(f"Insufficient elapsed warmup: {phase} {name}: {seconds}s")
    ...
    lines = ["# A-B-A benchmark comparison", "",
             ...
             "Sequential A1 → B → A2 on the same runner; 25 warmup and 15 measured 1-second iterations; no outlier removal.",
             "Means are ns/op; negative changes mean faster. Allocations are bytes/op.",
             "Candidate changes compare against each control separately. Drift is A2/A1 − 1.",
             "Acceptance: >5% improvement against both controls in an affected case, no >5% regression against both controls, no allocation growth.", "",
             "| Benchmark | A1 ns | B ns | A2 ns | B vs A1 | B vs A2 | A drift | A1/B/A2 bytes |",
             "|---|---:|---:|---:|---:|---:|---:|---:|"]
    for name in sorted(measured["A1"]):
        rows = [measured[phase][name] for phase in ("A1", "B", "A2")]
        a1, b, a2 = (row["Statistics"]["Mean"] for row in rows)
        change1 = (b / a1 - 1) * 100
        change2 = (b / a2 - 1) * 100
        drift = (a2 / a1 - 1) * 100
        ...
        lines.append(f"| {name.removeprefix('Kevlar.Benchmarks.')} | {a1:.2f} | {b:.2f} | "
                     f"{a2:.2f} | {change1:+.2f}% | {change2:+.2f}% | {drift:+.2f}%{flag} | {memory} |")
