...
$worktreeRoot = Join-Path (Split-Path $repo -Parent) "Kevlar-worktrees"
...
If the sandbox cannot write to the sibling directory, use `C:\tmp\Kevlar-worktrees` instead.
...
Kevlar has no Aspire AppHost. Do not run `aspire start`, `aspire stop`, or AppHost cleanup commands in this repo.

Unit and integration tests run in-process; integration coverage uses fakes plus loopback HTTP and does not require Docker. Docker is used only by `AgentLocks.ps1` for the shared Redis mutex. Do not stop or remove the `kevlar-agent-locks-redis` container while another loop may be active.
...
Kevlar tracks decomposition with GitHub **native sub-issues** (parent/child) and **dependencies** (`blocked_by` / `blocking`) when available, not markdown checklists as source of truth. Prefer native `blocked_by` relationships for blockage. Repo is `thomhurst/Kevlar`. Write calls need the numeric issue `.id` (an integer distinct from `#number`, passed with `-F`, not `-f`).
...
```powershell
gh api repos/thomhurst/Kevlar/issues/<N>/sub_issues --paginate --jq '.[] | "\(.number) [\(.state)] \(.title)"'
gh api repos/thomhurst/Kevlar/issues/<N>/dependencies/blocked_by --jq '[.[].number]'
```
...
3. Link the child to the parent as a native sub-issue:
   ...
   ```powershell
   gh api --method POST repos/thomhurst/Kevlar/issues/<N>/sub_issues -F sub_issue_id=$childId
   ```

4. For ordering, mark a child blocked by its prerequisite (use the prerequisite's `.id`):

   ```powershell
   gh api --method POST repos/thomhurst/Kevlar/issues/<blockedNum>/dependencies/blocked_by -F issue_id=<blockingId>
   ```
...
Search for the Shield API, strategy, execution path, extension package, analyzer, test, benchmark, or documentation journey the issue references. If it does not exist, the issue is asking you to build it; that is the true scope.

Use `rg` across source, tests, benchmarks, and docs before estimating scope:

```powershell
rg "<symbol|type|strategy|execution overload|option>" src tests benchmarks docs -g "*.cs" -g "*.md" -g "*.json" -g "*.yml"
```
...
4. Run focused tests, broader tests, and `npm run build --prefix docs` for docs changes. Kevlar uses TUnit executable projects: run `dotnet build Kevlar.slnx -c Release`, then `dotnet run --project tests/Kevlar.Tests -c Release --no-build -- --timeout 5m`, `dotnet run --project tests/Kevlar.IntegrationTests -c Release --no-build -- --timeout 5m`, and `dotnet run --project tests/Kevlar.Analyzers.Tests -c Release --no-build -- --timeout 5m` as relevant. For focus, pass one `--treenode-filter` pattern per command unless using valid TUnit OR parentheses.
...
- Preserve Kevlar's `netstandard2.0` and `net10.0` targets. Do not use APIs unavailable to `netstandard2.0` unless guarded by target-framework conditionals or supplied by existing compatibility packages.
- Cancellation is part of the public contract. Forward the supplied `CancellationToken` through async paths; changes to execution overloads must keep analyzer `KEV001` semantics correct.
- Shields are immutable and thread-safe; strategy state is intentionally shared when a Shield instance is reused. Changes to circuit breakers, rate/concurrency limiters, hedging, pooling, or composition need concurrency coverage.
- **Performance gate:** Kevlar is allocation-conscious. Changes touching Shield execution, strategy pipelines, `Outcome<T>`, pooled contexts, delay helpers, state-passing overloads, or HTTP handlers require a relevant before/after BenchmarkDotNet run (`dotnet run --project benchmarks/Kevlar.Benchmarks -c Release -- --filter "<filter>"`) with throughput and allocation numbers in the PR body. Material regressions are blockers.
- `/simplify` and review feedback must not introduce avoidable closures, boxing, `Task` allocations, async state machines, virtual dispatch, contention, or collection scans into hot paths. Preserve synchronous `ValueTask` fast paths and state-passing overloads.
- Integration tests use in-process fakes and loopback HTTP. They require no Docker service, but may need permission to bind a local port.
