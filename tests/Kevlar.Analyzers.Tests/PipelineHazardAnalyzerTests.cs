...
public class PipelineHazardAnalyzerTests
{
    ...
    public async Task KEV006_Skips_Typed_Shields_And_Typed_Builders()
    {
        var cases = new[]
        {
            ...
            "_ = Shield.For<int>().WhenResultEquals(0).Hedge(options => options.MaxHedgedAttempts = 1);",
    ...
    public async Task KEV003_Skips_Valid_Or_Unknown_Compositions()
    {
        var cases = new[]
        {
            ...
            "_ = Shield.For<int>().WhenResultEquals(0).Retry(1).Wrap(Shield.Empty).FallbackTo(0);",
    ...
    public async Task KEV010_Skips_Reference_Types_Nullables_Generic_Results_And_Explicit_Values()
    {
        var cases = new[]
        {
            ...
            "_ = Shield.For<int?>().WhenResultIsDefault().Retry(1);",
            "_ = Shield.For<int>().WhenResultEquals(0).Retry(1);",
            "_ = Build<int>();",
        };
    ...
    public async Task KEV012_Flags_Async_Hooks_Used_Synchronously()
    {
        var cases = new[]
        {
            ...
            "_ = ChaosShield.Fault(options => { options.Enabled = true; options.InjectionRateGenerator = async _ => { await Task.Yield(); return 1; }; }).Execute(_ => 1);",
            "_ = ChaosShield.Fault(options => { options.Enabled = true; options.ExceptionGenerator = async _ => { await Task.Yield(); return new Exception(); }; }).Execute(_ => 1);",
            "_ = ChaosShield.Latency(options => { options.Enabled = true; options.DelayGenerator = async _ => { await Task.Yield(); return TimeSpan.Zero; }; }).Execute(_ => 1);",
            "_ = ChaosShield.Outcome<int>(options => { options.Enabled = true; options.ResultGenerator = async _ => { await Task.Yield(); return 1; }; }).Execute(_ => 1);",
    ...
    public async Task KEV012_Skips_Async_Execution_Synchronous_Hooks_And_Unknown_Configuration()
    {
        var cases = new[]
        {
            ...
            "_ = ChaosShield.Fault(options => { options.Enabled = true; options.ExceptionGenerator = static _ => new(new Exception()); }).Execute(_ => 1);",
            "_ = ChaosShield.Latency(options => { options.Enabled = true; options.DelayGenerator = static _ => new(TimeSpan.Zero); }).Execute(_ => 1);",
            "_ = ChaosShield.Outcome<int>(options => { options.Enabled = true; options.ResultGenerator = static _ => new(1); }).Execute(_ => 1);",
            ...
            "_ = ChaosShield.Fault(options => { options.Enabled = false; options.ExceptionGenerator = async _ => { await Task.Yield(); return new Exception(); }; }).Execute(_ => 1);",
            "_ = ChaosShield.Outcome<int>(options => { options.Enabled = true; options.InjectionRate = 0; options.ResultGenerator = async _ => { await Task.Yield(); return 1; }; }).Execute(_ => 1);",
