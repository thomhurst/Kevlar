...
public class CircuitBreakerTests
{
    ...
    public async Task HalfOpen_Probe_Success_Closes_The_Circuit()
    {
        ...
        var probeResult = await policy.ExecuteAsync(_ => new ValueTask<int>(7));
        await Assert.That(probeResult).IsEqualTo(7);

        var subsequent = await policy.ExecuteAsync(_ => new ValueTask<int>(8));
    ...
    public async Task Monitor_Supports_Isolate_And_Reset()
    {
        ...
        var exception = await Assert.That(async () => await policy.ExecuteAsync(_ => new ValueTask<int>(1)))
            .Throws<CircuitOpenException>();
    ...
    public async Task Cancelled_Executions_Do_Not_Move_The_Circuit()
    {
        ...
        await Assert.That(async () => await policy.ExecuteAsync<int>(
            async token =>
            {
    ...
    {
        var policy = Policy.Handle<InvalidOperationException>().CircuitBreaker(1, TimeSpan.FromMinutes(1));

        // Unhandled exception types pass through without tripping the circuit.
        for (var i = 0; i < 3; i++)
        {
            await Assert.That(async () => await policy.ExecuteAsync<int>(_ => throw new ArgumentException()))
                .Throws<ArgumentException>();
        }

        var stillClosed = await policy.ExecuteAsync(_ => new ValueTask<int>(5));
