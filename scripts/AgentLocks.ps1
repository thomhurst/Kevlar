...
# resolves from -OwnerId, KEVLAR_AGENT_LOCK_OWNER_ID, or CODEX_THREAD_ID (in that order).
...
#   $env:KEVLAR_AGENT_LOCK_OWNER_ID = 'my-agent-run-id'
...
$redisContainer = 'kevlar-agent-locks-redis'
...
$stateRoot = Join-Path ([System.IO.Path]::GetTempPath()) 'kevlar-agent-locks'
...
function Resolve-OwnerIdentity([string]$Explicit) {
    ...
    if (-not [string]::IsNullOrWhiteSpace($env:KEVLAR_AGENT_LOCK_OWNER_ID)) {
        return $env:KEVLAR_AGENT_LOCK_OWNER_ID.Trim()
    }
...
if (-not $OwnerId) {
    Die 2 'cannot resolve lock owner identity: pass -OwnerId or set KEVLAR_AGENT_LOCK_OWNER_ID (CODEX_THREAD_ID is used automatically in Codex)'
}
...
$lockKey = "kevlar:agent-lock:$LockName"
$metaKey = "kevlar:agent-lock-meta:$LockName"
