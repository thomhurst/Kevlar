...
try {
    ...
    foreach ($file in @('docs/static/api/output.txt', 'docs/playwright-report/output.txt', 'docs/test-results/output.txt', 'BenchmarkResults/output.txt')) {
        Assert-True (Test-DisposableWorktreePath -Path $file) "Repository build output is not disposable: $file"
        $path = Join-Path $worktree $file
        New-Item -ItemType Directory -Path (Split-Path $path -Parent) -Force | Out-Null
        Set-Content -LiteralPath $path -Value 'generated output'
    }
